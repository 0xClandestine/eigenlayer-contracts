...
interface IAllocationManager is IAllocationManagerErrors, IAllocationManagerEvents, ISemVerMixin {
    ...
    /**
     * @notice Returns whether an operator is a valid slasher for an operator set.
     * @param operatorSet The operator set to query.
     * @param slasher The operator to query.
     * @dev If the slasher is not `active`, this will return false, even if they have been added as a slasher.
     */
    function isValidSlasher(OperatorSet memory operatorSet, address slasher) external view returns (bool);
     ...
     */
    function getAllSlashers(OperatorSet memory operatorSet) external view returns (address[] memory slashers, bool[] memory activeStatus);
}
