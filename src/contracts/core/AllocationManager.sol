    ...
    SemVerMixin
{
    ...
    {   
        ...
        require(_canCall(avs) || isValidSlasher(operatorSet, msg.sender), InvalidPermissions());
    ...
    /// @inheritdoc IAllocationManager
    function isValidSlasher(OperatorSet memory operatorSet, address slasher) public view returns (bool) {
    ...
    /// @inheritdoc IAllocationManager
    function getAllSlashers(OperatorSet memory operatorSet) external view returns (address[] memory, bool[] memory) {
        ...
        for (uint256 i = 0; i < slashers.length; i++) {
            activeStatus[i] = isValidSlasher(operatorSet, slashers[i]);
        }
